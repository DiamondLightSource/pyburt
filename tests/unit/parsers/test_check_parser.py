...
def test_base_case():
    ...
    check_parser = cp(tests.BLANK_REQ)
    assert tests.BLANK_REQ == check_parser.path
...
def test_multiline_comments():
    ...
    check_parser = cp(tests.MULTI_LINE_COMMENT_CHECK)
    with pytest.raises(ParserException):
        check_parser.parse()
...
def test_malformed_files():
    ...
    with pytest.raises(ParserException):
        check_parser = cp(tests.BAD_PREFIX_CHECK)
        check_parser.parse()

    with pytest.raises(ParserException):
        check_parser = cp(tests.BAD_VALUES_CHECK)
        check_parser.parse()

    with pytest.raises(ParserException):
        check_parser = cp(tests.EXCESS_VALUES_CHECK)
        check_parser.parse()

    with pytest.raises(ParserException):
        check_parser = cp(tests.EXTRA_PREFIX_CHECK)
        check_parser.parse()

    with pytest.raises(ParserException):
        check_parser = cp(tests.MISSING_TARGET_CHECK)
        check_parser.parse()
...
def test_non_floats():
    ...
    with pytest.raises(ParserException):
        check_parser = cp(tests.NORMAL_CHECK_1)
...
def test_check_parser_normal_1():
    ...
    check_parser = cp(tests.NORMAL_CHECK_1)
    assert tests.NORMAL_CHECK_1 == check_parser.path
...
def test_check_parser_normal_2():
    ...
    check_parser = cp(tests.NORMAL_CHECK_2)
    assert tests.NORMAL_CHECK_2 == check_parser.path
...
def test_check_parser_normal_3():
    ...
    check_parser = cp(tests.NORMAL_CHECK_3)
    assert tests.NORMAL_CHECK_3 == check_parser.path
